...
for model in models[-1:]:
    ...
    pipeline_str = f"""
      ...
      filesrc location={VIDEO_FILE} name=source_4 ! decodebin ! identity sync=true ! 
      queue name=start_queue_4 max-size-buffers=1 leaky=downstream ! 
      videoscale n-threads=2 ! videoconvert n-threads=2 qos=false ! 
      video/x-raw, pixel-aspect-ratio=1/1, format=RGB, width=640, height=640 ! 
      hailofilter name=set_src_4 so-path=/usr/lib/aarch64-linux-gnu/hailo/tappas/post_processes/libstream_id_tool.so config-path=src_2 ! 
      queue leaky=no max-size-buffers=1 ! robin.sink_4 
      
      filesrc location={VIDEO_FILE} name=source_5 ! decodebin ! identity sync=true ! 
      queue name=start_queue_5 max-size-buffers=1 leaky=downstream ! 
      videoscale n-threads=2 ! videoconvert n-threads=2 qos=false ! 
      video/x-raw, pixel-aspect-ratio=1/1, format=RGB, width=640, height=640 ! 
      hailofilter name=set_src_5 so-path=/usr/lib/aarch64-linux-gnu/hailo/tappas/post_processes/libstream_id_tool.so config-path=src_3 ! 
      queue leaky=no max-size-buffers=1 ! robin.sink_5 
      
      hailoroundrobin mode=1 name=robin ! 
      queue max-size-buffers=1 leaky=downstream ! 
      hailonet hef-path={MODEL_PATH} batch-size=6 vdevice-group-id=SHARED ! 
      queue max-size-buffers=1 leaky=downstream ! 
