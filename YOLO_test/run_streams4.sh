# =============================================
# ПАЙПЛАЙН ДЛЯ 4 ВИДЕОПОТОКОВ (batch-size=4)
# =============================================
...
#  hailonet name=inference_hailonet hef-path="$MODEL_PATH" batch-size=4  vdevice-group-id=SHARED ! \
...
#  fpsdisplaysink video-sink="fakesink sync=false" text-overlay=false silent=false sync=false 2> log4.txt
...
GST_DEBUG="fpsdisplaysink:5" gst-launch-1.0 \
 ...
 queue name=inference_hailonet_q leaky=no max-size-buffers=3 max-size-bytes=0 max-size-time=0  ! \
 hailonet name=inference_hailonet hef-path="$MODEL_PATH" batch-size=4  vdevice-group-id=SHARED output-format-type=HAILO_FORMAT_TYPE_FLOAT32 force-writable=true  ! \
 queue name=inference_hailofilter_q leaky=no max-size-buffers=3 max-size-bytes=0 max-size-time=0  ! \
 hailofilter name=inference_hailofilter so-path=/usr/local/hailo/resources/so/libyolo_hailortpp_postprocess.so function-name=filter_letterbox qos=false ! \
